...
exports.bancoPrecosItemPrecos = exports.bancoPrecosCotacaoItens = exports.bancoPrecosCotacoes = void 0;
/**
...
function setCors(req, res) {
    ...
    if (ALLOWED_ORIGIN.some(r => r.test(origin)))
...
function bpRequest(method, path, jwt, body) {
...
async function getJwt() {
    ...
    if (!token)
        throw new Error('Banco de Preços não retornou JWT (token inválido/revogado?).');
...
exports.bancoPrecosCotacoes = handler(async () => {
    ...
    const cotacoes = (Array.isArray(content) ? content : []).map((c) => ({
...
exports.bancoPrecosItemPrecos = handler(async (body) => {
    const idCotacao = Number(body.idCotacao);
    const idItem = Number(body.idItem);
    if (!idCotacao || !idItem)
        throw new Error('idCotacao e idItem obrigatórios.');
    const jwt = await getJwt();
...
//# sourceMappingURL=bancoDePrecos.js.map