 ...
 */
export const gerarPrestacaoContas = functions
  .runWith({ timeoutSeconds: 120, memory: '512MB', secrets: ['GEMINI_API_KEY'] })
  .https.onRequest(async (req, res) => {
    const origin = pickAllowedOrigin(req.headers.origin as string | undefined);
    res.set('Access-Control-Allow-Origin', origin);
    ...
    if (!titulo) { res.status(400).json({ error: 'tituloProjeto é obrigatório.' }); return; }
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) { res.status(500).json({ error: 'Servidor sem chave Gemini.' }); return; }

    const metasTxt = (arr: any[], rotulo: string) => Array.isArray(arr) && arr.length
      ? `${rotulo}:\n` + arr.map((m: any, i: number) => `  ${i + 1}. ${m?.meta || ''} — indicador: ${m?.indicador || ''}; verificação: ${m?.verificacao || ''}`).join('\n')
      : '';
    const ctx = [
      `PROJETO: ${titulo}`,
